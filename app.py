...
debug = True
showHeartbeat = True
# temp toggle for printing stuff while main debug toggle is off (clean up when done)
tempDebug = True
exitapp = False
firstHeartBeat = True
...
try:
    ...
    EnvView = os.environ.get('VIEW').split(",")
    _print("EnvView: " + str(EnvView))
except:
    EnvView = ''
...
# Initialize view based on Environment Variables
if EnvView is not None:
    view = sortIPs(list(EnvView))
    for i in view:
        if K < 1:
            replicas.append(i)
...
def heartBeat():
    ...
    for ip in notInView: #check if any nodes not currently in view came back online
        try:
            response = (requests.get((http_str + ip + kv_str + "get_all_replicas"), 
                timeout=2)).json()
            if response['result'] == 'success':
                # if(len(response['replicas']) < 2 and int(K) > 1):
                #     notInView.remove(ip)
                # else:
                notInView.remove(ip)
                view.append(ip)
                ...
                # if(view.index(ip) < K):
                #     try:
                #         response = (requests.put(http_str + ip_payload + kv_str + '_setIsReplica!', data={'id': 1, 'd': json.dumps(d),
                #             'vClock': json.dumps(vClock), 'storedTimeStamp': json.dumps(storedTimeStamp)}, timeout=2)).json()
...
def updateView(self, key):
    ...
    if _type == 'add':
        ...
        if ip_payload in view:
            ...
            # if numRep > 0:
            requests.put(http_str + ip_payload + kv_str + '_update!', data = {"K": K, "view": ','.join(view), 
                "notInView": ','.join(notInView), "replicas": ','.join(replicas), "proxies": ','.join(proxies)})
            ...
            if view.index(ip_payload) < K:
                ...
                requests.put(http_str + ip_payload + kv_str + '_setIsReplica!', data={'id': 1, 'd': json.dumps(d),
                    'vClock': json.dumps(vClock), 'storedTimeStamp': json.dumps(storedTimeStamp)}) #tell node to be replica
                ...
                # Creates new proxy
                requests.put(http_str + ip_payload + kv_str + '_setIsReplica!', data={'id': 0}) #tell node to be proxy
    ...
    if _type == 'remove':
        ...
        if ip_payload not in view:
            if ip_payload in notInView:
                notInView.remove(ip_payload)
                return {'result': 'success', 'number_of_nodes': len(view)}, 200
                return {'result': 'error', 'msg': 'Cannot remove node since it is not in view'}, 403
...
def updateRatio():
    ...
    for node in view:
        # This is a replica.
        if(int(K)==0):
            _print("K is 0 in updateRatio()")
            if node == IpPort:
                    isReplica = True
        elif view.index(node) < int(K):
            _print(str(node) + " is in 'elif' of updateRatio()")
    ...
    _print("K at end of updateRatio(): " + str(K))
...
class Handle(Resource):
    # global K, view, notInView, replicas, proxies, isReplica, d, vClock, storedTimeStamp
    if isReplica:
        #Handles GET request
        def get(self, key):
            global K, view, notInView, replicas, proxies, isReplica, d, vClock, storedTimeStamp
        ...
        def put(self, key):
            ...
            if key == '_setIsReplica!':
                try:
                    uni = request.form['id']
                    _print("in set replica, id = " + str(uni))
            ...
            _print("in Replica put after specials")
        ...
        def put(self, key):
            ...
            #Special command: adding/deleting nodes
            if key == 'update_view':
                return updateView(self, key)
            ...
            try:
                ...
                pass
            #Try requesting replicas until response or error if no responses
            noResp = True
            while noResp:
                if len(replicas) < 1:
                    return {'result': 'error', 'msg': 'Server unavailable'}, 500
